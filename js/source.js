...
let onSubmit = event => {
  ...
  if (
    xmlDoc
      .getElementsByTagName("ns2:OrderLine")[0]
      .getElementsByTagName("ns2:ProductType")[0].textContent === "Door"
  ) {
    ...
    switch (doorType) {
      case "F":
        renderFDoor({ doorWidth, doorHeight });
        break;
      case "E8":
        renderE8Door(event, context, height, width);
        break;
      case "2PA":
        render2PADoor({ doorHeight, doorWidth });
        break;
      case "loadImage":
        loadImage(event, context, height, width);
        break;
    }
  }
};
...
let renderE8Door = (event, domElement, doorHeight, doorWidth) => {
  let width = 400,
    height = 700;
  let newWidth = doorWidth * 10,
    newHeight = doorHeight * 10;
...
let render2PADoor = ({ doorHeight, doorWidth }) => {
  let width = 400,
    height = 700;
...
function drawRImExitDevice() {
  ...
  let {
    "surface-vert-b-gap": bGap,
    "surface-vert-l-gap": lGap,
    "surface-vert-r-gap": rGap
  } = document.forms.hw;
  ...
  /* draw big rect */
  const { hrRim, rimX, rimWidth, rimHeight } = drawHrRim({
    two,
    tGap,
    lGap,
    doorWidth,
...
let DrawMortiseLock = () => {
  ...
  let { mortiseBGap: bGap, mortiseRGap: rGap } = document.forms.hw;
  rGap = convertInchesIntoPix(rGap.value);
  bGap = convertInchesIntoPix(bGap.value);

  const baseWidth = Math.round(doorWidth * 0.1);
  ...
  const { mCircles, lgCircleY } = drawMortiseCircles({
    baseX,
    baseY,
    baseWidth,
    baseHeight
...
let SurfaceVerticalRod = () => {
  ...
  const {
    ...
    supX,
    supY
  } = drawUpperRod({ wingX2, wingWidth, wingY2, exitDeviceWidth, doorWidth });
  // draw lower rod with hatch
  const { lRodGrp } = drawLowerRod({
    uRodX1,
    uRodX3,
    wingY4,
    doorHeight,
...
document.addEventListener("DOMContentLoaded", () => {
  document
    .getElementById("door")
    .addEventListener("change", handleDoorTypeChangeHandler);
  document
    .getElementById("selectHW")
    .addEventListener("change", handeLockChangeHandler);
